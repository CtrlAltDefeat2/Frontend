...
export const watchListStyles = {
  ...
  moviesList: {
    ...
    card: {
      ...
      watched: 'border-border/40 opacity-75',
      unwatched: 'border-border/60 shadow-sm',
    },
    removeButton:
    ...
    watchedToggle: {
      ...
      watched: 'bg-green-500/90 text-white shadow-sm',
      unwatched: 'bg-black/60 text-white opacity-0 group-hover:opacity-100 hover:bg-primary',
      icon: 'h-3 w-3',
    },
    cover: {
    ...
    content: {
      ...
      title: {
        base: 'line-clamp-2 text-md font-semibold leading-tight',
        watched: 'text-muted-foreground line-through decoration-border',
      },
      meta: {
        container: 'mt-1 flex items-center justify-between',
        director: 'line-clamp-1 text-xs text-muted-foreground',
        year: 'text-[10px] text-muted-foreground border px-1 rounded',
      },
